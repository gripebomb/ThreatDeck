use ratatui::{Frame, layout::{Constraint, Direction, Layout}, style::{Style, Modifier}, widgets::{Block, Borders, Paragraph, Table, Row, Cell, Clear}};
...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let header = Row::new(vec!["Pattern", "Type", "Case", "Criticality", "Enabled", "Tags"])
        .style(Style::default().add_modifier(Modifier::BOLD).fg(app.theme.primary));

    let rows: Vec<Row> = app.keywords_list.iter().enumerate().map(|(i, k)| {
        ...
        let tag_str = match app.db.get_keyword_tags(k.id) {
            Ok(tags) => tags.iter().map(|t| t.name.clone()).collect::<Vec<_>>().join(", "),
            Err(_) => String::new(),
        };
        Row::new(vec![
            Cell::from(k.pattern.as_str()).style(Style::default().add_modifier(Modifier::BOLD)),
            Cell::from(format!("{:?}", k.match_type())),
            Cell::from(if k.case_sensitive { "Aa" } else { "Aa" }).style(Style::default().fg(if k.case_sensitive { app.theme.warning } else { app.theme.muted })),
            Cell::from(format!("{:?}", k.criticality)).style(Style::default().fg(crit_color).add_modifier(Modifier::BOLD)),
            Cell::from(if k.enabled { "✓" } else { "✗" }).style(Style::default().fg(if k.enabled { app.theme.success } else { app.theme.error })),
            Cell::from(tag_str),
        ]).style(style)
    }).collect();

    let table = Table::new(rows, vec![
        Constraint::Min(20), Constraint::Length(8), Constraint::Length(6),
        Constraint::Length(12), Constraint::Length(8), Constraint::Min(15),
    ])
    .header(header)
    ...
    let status_text = if app.keywords_show_form && app.input_mode == InputMode::Typing {
        ...
        "[a] Add  [e] Edit  [d] Delete  [t] Test  [Enter] Toggle  [q] Back".to_string()
    };
...
fn draw_form(f: &mut Frame, app: &App) {
    ...
    let title = if app.keywords_form_edit_id.is_some() { "Edit Keyword" } else { "Add Keyword" };
    ...
    let regex_label = if app.keywords_form.is_regex { "Regex" } else { "Simple" };
    draw_toggle_field(f, app, 1, "Match Type", regex_label, rows[2]);

    // Field 2: case_sensitive toggle
    let case_label = if app.keywords_form.case_sensitive { "Case Sensitive" } else { "Case Insensitive" };
    ...
    let enabled_label = if app.keywords_form.enabled { "Yes" } else { "No" };
...
fn draw_test_overlay(f: &mut Frame, app: &App) {
    ...
    let content = Paragraph::new("Keyword test mode placeholder. Type text to test against keywords.")
        .style(Style::default().fg(app.theme.fg));
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => {
            if !app.keywords_list.is_empty() {
                app.keywords_selected = (app.keywords_selected + 1).min(app.keywords_list.len() - 1);
            }
        }
        KeyCode::Up | KeyCode::Char('k') => {
            if app.keywords_selected > 0 {
                app.keywords_selected -= 1;
            }
        }
        _ => {}
    }
...
fn handle_form_normal_mode(app: &mut App, key: KeyEvent) {
    match key.code {
        ...
        KeyCode::Left | KeyCode::Right | KeyCode::Enter if app.form_focus == 3 => {
            cycle_criticality(&mut app.keywords_form.criticality, key.code == KeyCode::Right || key.code == KeyCode::Enter);
        }
...
fn submit_keyword_form(app: &mut App) {
    ...
    match res {
        Ok(_) => {
            ...
            app.form_focus = 0;
            app.refresh_keywords();
            app.set_notification("Keyword saved".to_string(), crate::types::NotificationType::Success);
        }
