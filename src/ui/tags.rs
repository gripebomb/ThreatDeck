...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let header = Row::new(vec!["Name", "Color", "Description", "Usage"])
        .style(Style::default().add_modifier(Modifier::BOLD).fg(app.theme.primary));

    let rows: Vec<Row> = app.tags_list.iter().enumerate().map(|(i, t)| {
    ...
    let table = Table::new(rows, vec![
        Constraint::Min(20), Constraint::Length(8), Constraint::Min(30), Constraint::Length(8),
    ])
    .header(header)
    ...
    let status_text = if app.tags_show_form && app.input_mode == InputMode::Typing {
        ...
        "[a] Add  [e] Edit  [d] Delete  [Enter] View items  [q] Back".to_string()
    };
...
fn draw_form(f: &mut Frame, app: &App) {
    ...
    let title = if app.tags_form_edit_id.is_some() { "Edit Tag" } else { "Add Tag" };
    ...
    // Field 1: color (text)
    draw_text_field(f, app, 1, "Color (hex, e.g. #ff0000)", &app.tags_form.color, rows[2]);

    // Field 2: description (text)
    draw_text_field(f, app, 2, "Description", &app.tags_form.description, rows[3]);
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => {
            if !app.tags_list.is_empty() {
                app.tags_selected = (app.tags_selected + 1).min(app.tags_list.len() - 1);
            }
        }
        KeyCode::Up | KeyCode::Char('k') => {
            if app.tags_selected > 0 {
                app.tags_selected -= 1;
            }
        }
        _ => {}
...
fn handle_form_normal_mode(app: &mut App, key: KeyEvent) {
    match key.code {
        ...
        KeyCode::BackTab => {
            app.form_focus = if app.form_focus == 0 { 2 } else { app.form_focus - 1 };
        }
        KeyCode::Esc => {
...
fn backspace_tag_field(app: &mut App) {
    match app.form_focus {
        0 => { app.tags_form.name.pop(); }
        1 => { app.tags_form.color.pop(); }
        2 => { app.tags_form.description.pop(); }
        _ => {}
    }
}
...
fn submit_tag_form(app: &mut App) {
    let create = crate::db::TagCreate {
        ...
        description: if app.tags_form.description.is_empty() { None } else { Some(app.tags_form.description.clone()) },
    };
    ...
    match res {
        Ok(_) => {
            ...
            app.form_focus = 0;
            app.refresh_tags();
            app.set_notification("Tag saved".to_string(), crate::types::NotificationType::Success);
        }
