use ratatui::{Frame, layout::{Constraint, Direction, Layout, Rect}, style::{Style, Modifier}, widgets::{Block, Borders, Paragraph, Table, Row, Cell, Clear}};
...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // Filter bar
    let filter_text = if app.feeds_filter.is_empty() { "Filter: (none)".to_string() } else { format!("Filter: {}", app.feeds_filter) };
    let filter_para = Paragraph::new(filter_text)
        .style(Style::default().fg(app.theme.muted));
    f.render_widget(filter_para, chunks[0]);

    // Table
    let header = Row::new(vec!["Status", "Name", "Type", "Interval", "Last Fetch", "Fails", "Tags"])
        .style(Style::default().add_modifier(Modifier::BOLD).fg(app.theme.primary));

    let rows: Vec<Row> = app.feeds_list.iter().enumerate().map(|(i, ft)| {
        let style = if i == app.feeds_selected {
        ...
        let tag_str = ft.tags.iter().map(|t| t.name.clone()).collect::<Vec<_>>().join(", ");
        let last_fetch = ft.feed.last_fetch_at.map(|dt| dt.format("%Y-%m-%d %H:%M").to_string()).unwrap_or_else(|| "Never".to_string());
        Row::new(vec![
    ...
    let table = Table::new(rows, vec![
        Constraint::Length(12), Constraint::Min(20), Constraint::Length(10),
        Constraint::Length(10), Constraint::Length(16), Constraint::Length(6), Constraint::Min(15),
    ])
    .header(header)
    ...
    let status_text = if app.input_mode == InputMode::Typing {
        ...
        "[a] Add  [e] Edit  [d] Delete  [m] Manual fetch  [t] Tags  [Enter] Toggle  [/] Filter  [s] Sort  [q] Back".to_string()
    };
...
fn draw_form(f: &mut Frame, app: &App) {
    ...
    let title = if app.feeds_form_edit_id.is_some() { "Edit Feed" } else { "Add Feed" };
    ...
    // Field 2: interval
    draw_form_field(f, app, 2, "Interval (sec)", &app.feeds_form.interval_secs.to_string(), rows[3]);
    // Field 3: api_key
    draw_form_field(f, app, 3, "API Key", &app.feeds_form.api_key, rows[4]);
    // Field 4: custom_headers
    draw_form_field(f, app, 4, "Custom Headers (JSON)", &app.feeds_form.custom_headers, rows[5]);
    // Field 5: tor_proxy
    ...
    draw_cycle_field(f, app, 7, "Feed Type", &app.feeds_form.feed_type.to_string(), rows[8]);
...
fn draw_form_field(f: &mut Frame, app: &App, field_idx: usize, label: &str, value: &str, area: Rect) {
...
fn draw_toggle_field(f: &mut Frame, app: &App, field_idx: usize, label: &str, value: &str, area: Rect) {
    let is_focused = app.form_focus == field_idx;
...
fn draw_cycle_field(f: &mut Frame, app: &App, field_idx: usize, label: &str, value: &str, area: Rect) {
    let is_focused = app.form_focus == field_idx;
    let border_color = if is_focused {
        app.theme.primary
    } else {
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char('m') => {
            app.set_notification("Manual fetch triggered".to_string(), crate::types::NotificationType::Info);
        }
        KeyCode::Char('t') => {
            app.set_notification("Tag assignment: select tags".to_string(), crate::types::NotificationType::Info);
        }
        KeyCode::Enter => {
            if let Some(ft) = app.feeds_list.get(app.feeds_selected) {
        ...
        KeyCode::Down | KeyCode::Char('j') => {
            if !app.feeds_list.is_empty() {
                app.feeds_selected = (app.feeds_selected + 1).min(app.feeds_list.len() - 1);
            }
        }
        KeyCode::Up | KeyCode::Char('k') => {
            if app.feeds_selected > 0 {
                app.feeds_selected -= 1;
            }
        }
        _ => {}
    }
...
fn handle_form_normal_mode(app: &mut App, key: KeyEvent) {
    match key.code {
        ...
        KeyCode::BackTab => {
            app.form_focus = if app.form_focus == 0 { 7 } else { app.form_focus - 1 };
        }
        KeyCode::Esc => {
        ...
        KeyCode::Left | KeyCode::Right | KeyCode::Enter if app.form_focus == 7 => {
            cycle_feed_type(app, key.code == KeyCode::Right || key.code == KeyCode::Enter);
        }
...
fn backspace_feed_field(app: &mut App) {
    match app.form_focus {
        0 => { app.feeds_form.name.pop(); }
        1 => { app.feeds_form.url.pop(); }
        2 => {
            // interval_secs: divide by 10 to remove last digit
            app.feeds_form.interval_secs /= 10;
        }
        3 => { app.feeds_form.api_key.pop(); }
        4 => { app.feeds_form.custom_headers.pop(); }
        5 => { app.feeds_form.tor_proxy.pop(); }
        _ => {}
    }
}

fn cycle_feed_type(app: &mut App, forward: bool) {
    let variants = [FeedType::Api, FeedType::Rss, FeedType::Website, FeedType::Onion];
...
fn submit_feed_form(app: &mut App) {
    let create = crate::db::FeedCreate {
        ...
        api_key: if app.feeds_form.api_key.is_empty() { None } else { Some(app.feeds_form.api_key.clone()) },
        custom_headers: if app.feeds_form.custom_headers.is_empty() { None } else { Some(app.feeds_form.custom_headers.clone()) },
        tor_proxy: if app.feeds_form.tor_proxy.is_empty() { None } else { Some(app.feeds_form.tor_proxy.clone()) },
    };
    ...
    match res {
        Ok(_) => {
            ...
            app.form_focus = 0;
            app.refresh_feeds();
            app.set_notification("Feed saved".to_string(), crate::types::NotificationType::Success);
        }
