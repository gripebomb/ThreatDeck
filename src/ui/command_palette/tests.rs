    use crate::ui::command_palette::{
        CommandPaletteState, PaletteMode, CommandId,
        registry, matcher,
    };
    ...
    fn all_commands_have_titles() {
        for cmd in registry::ALL_COMMANDS {
            assert!(!cmd.title.is_empty(), "Command {:?} has empty title", cmd.id);
        }
    }
    ...
    fn contextual_commands_filtered_correctly() {
        ...
        assert!(
            available_with_feed.iter().any(|c| c.id == CommandId::FeedEditSelected),
    ...
    fn normalize_strips_colon() {
        assert_eq!(matcher::normalize_input(":feed check all"), "feed check all");
    }
    ...
    fn search_by_canonical() {
        ...
        assert!(
            results.iter().any(|m| m.command.id == CommandId::FeedCheckAll),
    ...
    fn multi_token_match() {
        ...
        assert!(
            results.iter().any(|m| m.command.id == CommandId::NotifyTestDiscord),
